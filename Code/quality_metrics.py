...
def write_positions_and_knn(L, numb_points, directory, k=10, radius=0.1, proximity_mode="knn", mode_3D=True,
    ...
    distances, indices, indices_knn = [element.tolist() for element in distances], [element.tolist() for element in indices], [element.tolist() for element in indices_knn]
    indices_knn = [[index + 1 for index in vector] for vector in indices_knn]  # Avoid starting at 0
    ...
    write_graph_df(directory, "knn", numb_points, dim, positions, indices_knn, distances, weights)
    if proximity_mode != "knn":
        write_graph_df(directory, proximity_mode, numb_points, dim, positions, indices, distances, weights)

def write_graph_df(directory, proximity_mode, numb_points, dim, positions, indices, distances, weights):
